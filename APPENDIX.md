...
$$
x=c_{g}+\epsilon_{m},E[\epsilon_{m}]=0,\mathrm{Cov}(\epsilon_{m})=\frac{\Sigma_{g}}{m} \qquad (\text{D.1})
$$
...
$$
z_{h}- z_{v' }=(c_{h}- c_{g})+(\epsilon_{h}- \epsilon_{v' }) \qquad (\text{D.5})
$$
...
$$
E[I]=\mathrm{tr}(J\mathrm{Cov}(\epsilon_{v}){J}^{T}) \qquad (\text{D.17})
$$
...
$$
z_{g}- z_{h}=(f(c_{g})- f(c_{h}))+(J_{g}\epsilon_{g}- J_{h}\epsilon_{h}) \qquad (\text{D.21})
$$
...
$$
z_{g}- z_{h}=f(c_{g})- f(c_{h}) \qquad (\text{D.22})
$$
...
$$
\nabla_{a_{g}}\ell_{\mathrm{CE}}=- \frac{1- p_{g}}{\tau }z_{v},\nabla_{a_{h}}\ell_{\mathrm{CE}}=\frac{p_{h}}{\tau }z_{v} \qquad (\text{D.23})
$$
...
$$
\nabla_{a_{g}}(\alpha \ell_{\mathrm{align}})=- \alpha z_{v} \qquad (\text{D.24})
$$
...
$$
\Theta (T\cdot (G+\kappa \cdot w)\cdot m_{a}\cdot Q\cdot d) \qquad (\text{H.3})
$$
...
$$
\mathrm{tag}\approx 0.735- 0.32\times \mathrm{margin} \qquad (\text{J.1})
$$
...
$$
\mathrm{tag}\approx 0.762- 0.39\times \mathrm{margin} \qquad (\text{J.2})
$$
